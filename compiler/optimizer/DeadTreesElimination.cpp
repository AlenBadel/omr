...
typedef TR::typed_allocator<TR::TreeTop *, TR::Region&> ReadBarToTreeTopDequeAlloc;
typedef std::deque<TR::TreeTop *, ReadBarToTreeTopDequeAlloc> ReadBarToTreeTopMap;
...
void TR::DeadTreesElimination::prePerformOnBlocks()
   ...
   ReadBarToTreeTopMap rdbar2ttMap(ReadBarToTreeTopDequeAlloc(comp()->trMemory()->currentStackRegion()));
   TR_BitVector visitedReadbarsUnderTreeTop(comp()->trMemory()->currentStackRegion());
   ...
   for (TR::TreeTop *tt = comp()->getStartTree();
      ...
      else if (node->getOpCode().isAnchor() && visitedReadbarsUnderTreeTop.isSet(node->getFirstChild()->getGlobalIndex()))
         {
         TR::TreeTop *ttToRemove = rdbar2ttMap[node->getFirstChild()->getGlobalIndex()];
         if (performTransformation(comp(), "%sRemove trivial dead tree (rdbar under treetop before compressedrefs or any checks): %p\n", optDetailString(), ttToRemove->getNode()))
      ...
      if (!removed && isReadBarreirUnderTreetop(node))
         {
         ncount_t nodeIndex = node->getFirstChild()->getGlobalIndex();
         if (nodeIndex + 1 > rdbar2ttMap.size())
            rdbar2ttMap.resize(nodeIndex+1);
         rdbar2ttMap[nodeIndex] = tt;
         visitedReadbarsUnderTreeTop.set(nodeIndex);
         }
